...
export default function HomeScreen({ route, navigation }) {
  ...
  // Listen to navigation events to update activeTab
  useEffect(() => {
    const unsubscribe = navigation.addListener('state', () => {
      const { name } = navigation.dangerouslyGetState().routes.slice(-1)[0];
      setActiveTab(name);
    });
  ...
  return (
    <Background>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <BottomNavigationBar userName={userName} activeTab={activeTab} setActiveTab={setActiveTab} />
      </ScrollView>
    </Background>
  );
}
